...
if [ -z "$1" ] || [ "$1" == 'generator' ]; then
  ...
  assertDir 'src' $ERROR
  assertDir 'package.json' $ERROR
  assertDir 'composer.json' $ERROR
  assertDir 'docker-compose.yml' $ERROR
  assertDir 'build.yml' $ERROR
fi
...
if [ -z "$1" ] || [ "$1" == 'gdt' ]; then
  ...
  assertDir 'build/html/index.php' $ERROR
fi
